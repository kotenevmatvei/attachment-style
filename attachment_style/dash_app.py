...
questions = combine_and_shuffle_lists(
        anxious_questions,
        secure_questions,
        avoidant_questions
    )
...
app.layout = dbc.Container([
    ...
    dbc.Row(dbc.Col(dbc.Collapse(html.Div(f"Question {question_count}/42", className="mb-2"), id="question", is_open=False), className="text-center")),
...
def update_question(n_submit, question_count):
    if n_submit is not None and question_count < len(questions):
        question_count += 1
    return question_count, questions[question_count - 1] if question_count <= len(questions) else "No more questions"
